...
import { ResponsiveContainer, Legend, LineChart, CartesianGrid, XAxis, YAxis, Tooltip, Line, TooltipContentProps } from "recharts";
...
export default function DemoChart({ logs, userExercises, units, dateRange, setDateRange, selectedExercise, setSelectedExercise }: Props) {
    ...
    const chartData = useMemo((): ChartData => {
        ...
        const exerciseLogs: ExerciseLog[] = filteredLogs.map((log) => {
            ...
            const convertedOrm = exercise?.orm != null ? convertFromBase(exercise.orm) : null;
        ...
        const orms = filledLogs.map((l) => l.orm).filter((v): v is number => v != null);
        let paddedOrmMin = 0, paddedOrmMax = 0;

        if (orms.length) {
            const minOrm = Math.min(...orms);
            const maxOrm = Math.max(...orms);
            paddedOrmMin = Math.floor(minOrm / 10) * 10 - 10;
            paddedOrmMax = Math.ceil(maxOrm / 10) * 10 + 10;
        }
        ...
        return {
            ...
            exDomain: { exMin: paddedOrmMin, exMax: paddedOrmMax },
            yTickCount,
        };
        ...
        <div className="dashboard-section-1 rounded-lg shadow lg:w-5/10">
            ...
            <ResponsiveContainer width="100%" height={isMobile ? 200 : 300}>
                ...
                >
                    ...
                    <Line yAxisId="right" type="monotone" dataKey="orm" stroke="red" strokeWidth={3} dot={{ r: 1 }} connectNulls />
